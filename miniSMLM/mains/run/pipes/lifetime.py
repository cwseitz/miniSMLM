...
class TwoStatePoissonHMM:
    def __init__(self,stack):
        self.stack=stack
        
    def fit_hmm(self,data):
        model = hmm.PoissonHMM(n_components=2,random_state=None,n_iter=10)
        model.fit(data)
    ...
    def plot_hmm(self,data,states,model):
        ...
        ax[0].plot(time,data,color='black',alpha=0.5)
    ...
    def plot_hist(self,life0,life1):
        """currently only for two states system"""
        def func_doublexp(x, m, c0):
            return c0 * np.exp(m*x)
        fig,ax=plt.subplots(figsize=(3,3))
        bins = np.arange(1,20,1)*0.01
        vals0,bins0 = np.histogram(life0,bins=bins,density=False)
        vals0 = vals0/np.sum(vals0)
        opt0, cov0 = curve_fit(func_doublexp,bins0[:-1],vals0)
        vals1,bins1 = np.histogram(life1,bins=bins,density=False)
    ...
    def forward(self,show_spots=False,threshold=0.0007,fps=100,
                show_hmm=False,show_hist=False):
        ...
        for n in range(nspots):
            ...
            model = self.fit_hmm(this_data)
            states = model.predict(this_data)
            lambdas = np.sort(model.lambdas_,axis=0)
            rate = model.lambdas_[states]
            lam0,lam1 = np.squeeze(lambdas)
            this_data0 = this_data[np.argwhere(rate == lam0)]
            this_data1 = this_data[np.argwhere(rate == lam1)]
            _life0 = self.lifetime(rate,lam0)/fps
            _life1 = self.lifetime(rate,lam1)/fps
            if show_hmm:
        ...
        print(f'Mean OFF life {1e3*np.mean(life0)} ms')
        print(f'Mean ON life {1e3*np.mean(life1)} ms')
        
        if show_hist:
            self.plot_hist(life0,life1)
